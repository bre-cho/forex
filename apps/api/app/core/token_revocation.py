...
def normalize_iat(token_iat: int | float | str | datetime | None) -> int | None:
    if token_iat is None:
        return None
    if isinstance(token_iat, datetime):
        return int(token_iat.timestamp())
    try:
        return int(token_iat)
...
async def revoke_all_user_access_tokens(user_id: str) -> None:
    ...
    await redis.set(f"{_USER_ACCESS_REVOKED_AFTER_PREFIX}{user_id}", int(time.time()))
